...
**a bis) Tests des écritures de journal (ISA 240).** Depuis l'écran **Écritures de journal**, lancez l'analyse : Probare parcourt **tout le grand livre** et attribue à chaque écriture un **score de risque** (déséquilibre débits/crédits, montant juste sous le seuil de signification, contrepartie inhabituelle — un produit ou une charge directement soldé par la trésorerie sans passer par un tiers —, écriture datée un week-end, écriture de clôture tardive, libellé absent ou générique…). Les écritures les plus risquées sont retenues pour **revue ciblée** : dépliez chacune, investiguez, puis pointez-la **conforme** ou **anomalie** avec votre constat. Chaque type de signal génère une exception à trancher comme les autres. Cette diligence est obligatoire sur toute mission (risque de contournement des contrôles par la direction).
